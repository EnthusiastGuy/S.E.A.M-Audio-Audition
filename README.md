...
- **Loads structured sample packs** — Expect a root folder that contains song subfolders with `.wav` files. Songs will be directly inside the selected root. Inside each song folder, WAV files follow a simple naming scheme so the app can infer **parts**, **branch targets**, and optional **full-mix** files.
